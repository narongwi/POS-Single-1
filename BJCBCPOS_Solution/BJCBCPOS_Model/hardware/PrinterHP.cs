...
{
    public class PrinterHP: IHardware
    {
        private clsOPOSHP opos = null;
        private clsOPOSHP.DrawerStatusEventHandler drawerCurrentEvent = null;
        ...
        public PrinterHP(string key)
        {
            try
            {
                AppLog.writeLog("Initial Hardware with BCOPOSHP.dll");
                if (opos == null)
                {
                    opos = new clsOPOSHP(key);
                    drawerChecked = false;
        ...
        public bool checkCashDrawer()
        {
            try
            {
                if (!drawerChecked)
                {
                    ...
                    opos.DrawerStatus += new clsOPOSHP.DrawerStatusEventHandler(drawerListenersStatus);
                    drawerChecked = true;
                }
        ...
        public void addDrawerListeners(Hardware.DrawerListener listener)
        {
            try
            {
                ...
                // add new event
                drawerCurrentEvent = new clsOPOSHP.DrawerStatusEventHandler(listener);
                opos.DrawerStatus += drawerCurrentEvent;
            }
            catch (Exception ex)
            {
