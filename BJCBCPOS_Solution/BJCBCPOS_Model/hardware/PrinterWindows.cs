...
{
    public class PrinterWindows : IHardware
    {
        private static clsOPOSWin opos = null;
        private static clsOPOSWin.DrawerStatusEventHandler drawerCurrentEvent = null;
        ...
        public PrinterWindows(string key)
        {
            try
            {
                AppLog.writeLog("Initial Hardware with BCOPOSWIN.dll");
                if (opos == null)
                {
                    opos = new clsOPOSWin(key);
        ...
        public bool checkCashDrawer()
        {
            try
            {
                if (!drawerChecked)
                {
                    ...
                    opos.DrawerStatus += new clsOPOSWin.DrawerStatusEventHandler(drawerListenersStatus);
                    drawerChecked = true;
                }
        ...
        public void addDrawerListeners(Hardware.DrawerListener listener)
        {
            try
            {
                ...
                // add new event
                drawerCurrentEvent = new clsOPOSWin.DrawerStatusEventHandler(listener);
                opos.DrawerStatus += drawerCurrentEvent;
            }
            catch (Exception ex)
            {
